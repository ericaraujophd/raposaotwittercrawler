...
def get_mongodb_uri(config_f='../settings/mongodb_config.txt'):
    try:
...
def get_mongodb_local_url(config_f='../settings/mongodb_local_config.txt'):
    try:
...
def connect(local=False):
    if not local:
        ...
        try:
            ...
            mongodb_url_str = 'mongodb://{0}:{1}@127.0.0.1'.format(mongodb_dict['username'], mongodb_dict['password'])
